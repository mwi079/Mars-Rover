...
function parseOutput(roverData){
    ...
    for (const [index,rover] of roverData.entries()){
        if(rover.lost && index<roverData.length-1) result=result.concat(`(${rover.x}, ${rover.y}, ${rover.direction}) 'LOST'\n`)
        else if (!rover.lost && index<roverData.length-1) result=result.concat(`(${rover.x}, ${rover.y}, ${rover.direction})\n`)
        else if (rover.lost && index===roverData.length-1) result=result.concat(`(${rover.x}, ${rover.y}, ${rover.direction}) 'LOST'`)
        else  result=result.concat(`(${rover.x}, ${rover.y}, ${rover.direction})`)
    }
    return result
}
